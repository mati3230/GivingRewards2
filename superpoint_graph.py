...
class SuperpointGraph:
    ...
    def __init__(self, pns_orig=None, sort_mode=False):
        ...
        pns_orig: list
            will be a tuple where the first element is a list of
            points idxs of the superpoints and the second element is a list of
            neighbour superpoints of the superpoints (pns is shorthand for
            'p'oints indices and 'n'eighbour's' of a superpoint)
        sort_mode: boolean
            if True, an ascending sortation according to the sizes of the
    ...
    def remove_idx(self, idx):
        del self._pns[0][idx]
        del self._pns[1][idx]
        for j in range(len(self._pns[1])):
            nbs = self._pns[1][j]
            idxs_to_change = np.where(nbs >= idx)[0]
            nbs[idxs_to_change] -= 1
            self._pns[1][j] = nbs

    def remove(self, blacklist):
        """
        Remove blacklisted superpoints from the graph.

        Parameters
        ----------
        blacklist : list(int)
            List of superpoint indices which should be removed.
        """
    ...
    def save(self, cdir, id):
        ...
        """
        #print("save", len(self._pns[0]))
        tmp_pns = [list(), list()]
        for i in range(len(self._pns[0])):
            tmp_pns[0].append(self._pns[0][i].tolist())
            tmp_pns[1].append(self._pns[1][i].tolist())

        with open(cdir+id+"/pns_orig.json", "w") as outfile:
            json.dump(tmp_pns, outfile, indent=4)

    def load(self, cdir, id):
        """
        ...
        """
        self._pns_orig = [list(), list()]
        with open(cdir+id+"/pns_orig.json") as infile:
    ...
    def get_sp_point_idxs(self, idx):
        ...
        try:
            ...
            print("query idx:", idx, "len pns:", len(self._pns), "len pns[0]:", len(self._pns[0]))
            raise
        if P_idxs.shape[0] == 0:
            raise Exception("Requested idxs with zero points at idx " + str(idx))
        return P_idxs
    ...
    def _replace(self, old_idx, new_idx):
        ...
        for i in range(n_superpoints):
            ...
            if self._sort_mode:
                lengths = self._lengths[n_idxs]
                sortation = np.argsort(lengths)
                n_idxs = n_idxs[sortation]
    ...
    def unify(self, superpoint_idx_a, superpoint_idx_b):
        ...
        if P_idxs_a.shape[0] == 0:
            print(P_idxs_b.shape)
            raise Exception("SP with zero points unified")
    ...
    def break_all_connections(self, superpoint_idx):
        ...
        usually called if a certain superpoint should is deleted, e.g., when two
    ...
    def get_n_neighbours(self, to_do):
        edges = []
        for i in range(len(to_do)):
            neighbours = self._pns[1][i]
            for j in range(len(neighbours)):
                neighbour = neighbours[j]
                if neighbour in self._blacklist[i]:
                    continue
                edge = {i, neighbour}
                if edge in edges:
                    continue
