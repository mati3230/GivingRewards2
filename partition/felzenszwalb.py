...
def min_internal_difference(G, S, C1, C2, k):
    ...
    IntS_i = internal_difference(G, S, C1)
    IntS_j = internal_difference(G, S, C2)
    tau_i = tau(k, S, C1)
    tau_j = tau(k, S, C2)
    arg_i = IntS_i + tau_i
    arg_j = IntS_j + tau_j
    #print("{0:.3f}, {1:.3f}, {2:.3f}, {3:.3f}, {4:.3f}, {5:.3f}".format(
    #    IntS_i, IntS_j, tau_i, tau_j, arg_i, arg_j))
    return min(arg_i, arg_j)
...
def merge(G, w, C1, C2, S, e, k):
    ...
    #print(C1, C2, MInt, w)
    #print("{0}, {1:.3f}, {2:.3f}".format(w>MInt, w, MInt))
    if w > MInt:
        return S
    i, j = e
    S[j] = S[i]
