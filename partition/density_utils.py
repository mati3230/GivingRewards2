...
def densities_np_osize(uni_a, uni_b, p_a, p_b, indices_a, counts_a, return_differences=False):
    ...
    if return_differences:
        differences = np.ones(densities.shape)
        differences = differences - densities
        return densities, differences
    return densities


def densities_np(uni_a, uni_b, p_a, p_b, indices_a, counts_a, return_differences=False):
    """ Compute the densities between a partition A and B with a numpy function. 
    ...
    if return_differences:
        differences = np.tile(counts_a[None, :], (densities.shape[0], 1))
        differences = differences - densities
        return densities, differences
    return densities
